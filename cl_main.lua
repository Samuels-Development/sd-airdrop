...
RegisterNetEvent('crate:clearcrate', function()
    crate = nil
end)
...
function OpenCrate(entity, crate, item, amount)
    ...
    TriggerServerEvent('crate:deleteCrate', netId)

    TriggerServerEvent("kudos-supplydrop:server:ItemHandler", "add", item, amount)                           
...
-- Remove Crate
RegisterNetEvent("crate:removeCrate")
AddEventHandler("crate:removeCrate", function(entity)
    DeleteEntity(entity)
end)

-- Apply Natives
RegisterNetEvent("crate:applyNatives")
AddEventHandler("crate:applyNatives", function(netId)
...
function CrateDrop(item, amount, planeSpawnDistance, dropCoords)
    CreateThread(function()
        ...
        SetTimeout(2000, function()            	               
            ...
            TriggerServerEvent("crate:spawnCrate", crateSpawn, item, amount)
            while crate == nil do
                Wait(250)
            end
...
RegisterNetEvent("crate:createQbTarget")
...
-- Start the AirDrop
RegisterNetEvent("kudos-supplydrop:client:StartDrop", function(item, amount, roofCheck, planeSpawnDistance, dropCoords)
    CreateThread(function()          
...
-- Create the AirDrop
RegisterNetEvent("kudos-supplydrop:client:CreateDrop", function(useditem, roofCheck, planeSpawnDistance)
    local playerCoords = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 10.0, 0.0)        
    local item, amount = GetRandomItemData(useditem)
    QBCore.Functions.TriggerCallback("kudos-supplydrop:server:cooldown", function(cooldown) 
        if not cooldown then
            QBCore.Functions.TriggerCallback("kudos-supplydrop:server:getCops", function(CurrentCops)
                if CurrentCops >= Config.RequiredCops then
                    TriggerServerEvent('kudos-supplydrop:server:startCooldown')              
                    ...
                    PoliceAlert()    
                    TriggerServerEvent("kudos-supplydrop:server:ItemHandler", "remove", useditem, 1)
                    TriggerEvent("kudos-supplydrop:client:StartDrop", item, amount, roofCheck or false, planeSpawnDistance or 400.0, vector3(playerCoords.x, playerCoords.y, playerCoords.z))                
                else
